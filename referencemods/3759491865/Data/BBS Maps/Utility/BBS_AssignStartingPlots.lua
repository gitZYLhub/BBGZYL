...
function BBS_AssignStartingPlots:__RateBiasPlots(biases, startPlots, major, region_index, civilizationType,iPlayer)
    ...
    for i, plot in ipairs(startPlots) do
        ...
        if (major) then
            ...
            -- 所有无海岸关联的文明不允许出生在海边
            if SEAS_CIVILIZATION[civilizationType] then
            ...
            if (not foundBiasToundra) then
                ...
                local fTundraLatitude = 0.77;

                if ratedPlot.Plot:GetY() / g_iH > 0.78 or ratedPlot.Plot:GetY() / g_iH < 0.13 then
                    tempTundra = math.abs(ratedPlot.Plot:GetY() / g_iH - 0.5) * 20;
                end
