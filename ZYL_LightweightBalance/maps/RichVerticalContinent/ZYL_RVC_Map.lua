...
function BBS_Assign(args)
	...
	print("Rich Vertical Continent: assigning starting plots")
...
function GenerateMap()
	...
	print("Rich Vertical Continent: generation started")
	local pPlot;
	...
	print("Rich Vertical Continent: generation complete")
end
...
function TeamPVPGenerateContinents(plotTypes)
	...
	local mapRow = ZYL_RVC_GetMapRow();
	local MapContinentsNum = math.max(1, (mapRow and mapRow.Continents or 1) + math.floor(RichNum / 3) - 1);
	-- 陆地总数
	local Lands  = 0;
	...
	local LandsAvg = math.max(1, math.ceil(Lands / MapContinentsNum));
	-- 将两侧的陆地分配到同一个大陆
