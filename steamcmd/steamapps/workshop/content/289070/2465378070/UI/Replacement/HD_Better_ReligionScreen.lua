print("Override HD_Better_ReligionScreen.lua for adapt Better Religion Screen");
...
function GetBeliefsSortIndex(row)
	local belief = GameInfo.BeliefsSortIndex[row.BeliefType];
	if belief ~= nil then
...
function PopulateAvailableBeliefs(beliefType:string)
	...
	for row in GameInfo.Beliefs() do
		...
		if (not bBeliefTypeAlreadySelected and 
			...
			table.insert(available_Beliefs, row);
		end
	end

	-- 信条排序
	table.sort(available_Beliefs, function(a, b)
		return GetBeliefsSortIndex(a) < GetBeliefsSortIndex(b);
		end );

	-- 创建信条实例
	for _, row in ipairs(available_Beliefs) do
		...
		-- 	print("信条已被选取", Locale.ToUpper(row.Name))
...
function ViewReligion(religionType:number)
  ...
  tReligionBeliefs[1] = pantheonBelief;
	for _, beliefIndex in ipairs(religion.Beliefs) do
		belief = GameInfo.Beliefs[beliefIndex];
    -- print("已选信条", LL(belief.Name))
    if     belief.BeliefClassType == "BELIEF_CLASS_FOLLOWER" then
      if tReligionBeliefs[2] == -1 then
        tReligionBeliefs[2] = beliefIndex;
      else
        table.insert(tReligionBeliefs, beliefIndex)
      end
    elseif belief.BeliefClassType == "BELIEF_CLASS_WORSHIP"  then
      if tReligionBeliefs[3] == -1 then
        tReligionBeliefs[3] = beliefIndex;
      else
        table.insert(tReligionBeliefs, beliefIndex)
      end
    elseif belief.BeliefClassType == "BELIEF_CLASS_FOUNDER"  then
      if tReligionBeliefs[4] == -1 then
        tReligionBeliefs[4] = beliefIndex;
      else
        table.insert(tReligionBeliefs, beliefIndex)
      end
    elseif belief.BeliefClassType == "BELIEF_CLASS_ENHANCER" then
      if tReligionBeliefs[5] == -1 then
        tReligionBeliefs[5] = beliefIndex;
      else
        table.insert(tReligionBeliefs, beliefIndex)
      end
    end
    ...
    for i = 2, #tReligionBeliefs do AddSingleBelief(tReligionBeliefs[i], religion.Founder == localPlayerID); end
