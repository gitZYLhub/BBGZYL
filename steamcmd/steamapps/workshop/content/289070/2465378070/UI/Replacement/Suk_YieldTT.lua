...
print("HD override Suk_YieldTT.lua")
...
function TTtoTable(sTT)
	...
	for _, sLine in ipairs(split(sTT, "%s*%[NEWLINE%]%s*")) do
		...
		-- print('=====================================')
		-- print(sLine)
		-- print(tLine[1]) -- 一级标题的数值
		-- print(tLine[2]) -- 二级标题的数值
		-- print(tLine[3]) -- 文本
		
		-- 标记是否成功进行了合并
		local combine_flag = false;
		-- 对于二级标题，遍历寻找可以合并的项目
		if tLine[1] == '' and tLine[2] ~= '' then
			
			-- 倒序遍历现有的列表
			local currentLength = #tTT;
			for i = currentLength, 1, -1 do
				-- 遇到第一个一级标题，直接退出循环
				if tTT[i][1] ~= '' then break; end

				-- 判断是否是同名
				if tTT[i][2] ~= '' and tLine[3] == tTT[i][3] then
					-- 找到第一个同名的二级标题，合并数据，然后退出循环
					local lastNum = tonumber(tTT[i][2]);
					local nowNum = tonumber(tLine[2])
					local totalNum = lastNum + nowNum;
					if totalNum >= 0 then
						tTT[i][2] = '+' .. totalNum;
					else
						tTT[i][2] = tostring(totalNum);
					end
					combine_flag = true;
					break;
				end
