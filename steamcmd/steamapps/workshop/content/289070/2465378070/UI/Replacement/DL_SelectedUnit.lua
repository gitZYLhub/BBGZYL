    ...
    if kUnit ~= nil and ( not UI.IsGameCoreBusy() ) then
        ...
        if playerID == Game.GetLocalPlayer() then
            ...
            elseif GameInfo.Units[kUnit:GetUnitType()].ParkCharges > 0 and kUnit:GetParkCharges() > 0 then -- Highlight plots that can activated by Naturalists
                ...
                UILens.ToggleLayerOn(m_HexColoringGreatPeople);
            elseif kUnitRockBand ~= nil and GameInfo.Units[kUnit:GetUnitType()].UnitType == "UNIT_ROCK_BAND"
                                        or GameInfo.Units[kUnit:GetUnitType()].UnitType == "UNIT_ORAL_SCHOLAR" 
                                        or GameInfo.Units[kUnit:GetUnitType()].UnitType == "UNIT_TRAVEL_THEATRE"
                                        or GameInfo.Units[kUnit:GetUnitType()].UnitType == "UNIT_MALI_GRIOTS" then -- Highlight plots that can activated by RockBands
