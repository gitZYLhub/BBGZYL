...
function Timer1Stop(txt:string)
	if m_NumTicks1 == 0 then
    -- print("Timer1: no ticks");
    return;
  end
...
function Timer2Stop(txt:string)
	if m_NumTicks2 == 0 then
    -- print("Timer2: no ticks");
    return;
  end
...
function FetchAndCacheDataReq(sReqId:string)
	...
	-- Check if exists
	if not req then
    -- print("ERROR: FetchAndCacheDataReq unknown requirement", sReqId);
    return nil;
  end
...
function FetchAndCacheDataReqSet(sReqSetId:string)
	...
	if not req then
    -- print("ERROR: FetchAndCacheDataReqSet unknown req set", sReqSetId);
    return nil;
  end
	tReqSet = {};
...
function FetchAndCacheData(sModifierId:string)
	...
	-- check if it exists!
	if not mod then
    -- print("WARNING! FetchAndCacheData: No definition for Modifier", sModifierId);
    return nil;
  end
	...
	if tModifier.CollectionType == nil or tModifier.EffectType == nil then
    -- print("WARNING! FetchAndCacheData: No dynamic modifier definition for Modifier", sModifierId);
    return nil;
  end
...
function CheckOneRequirement(tReq:table, tSubject:table, sSubjectType:string)
	...
	local function CheckForMismatchError(sExpectedType:string, sExpectedType2:string)
		...
		-- print("ERROR: CheckOneRequirement mismatch for exp subject", sExpectedType, "got", tSubject.SubjectType, "req is", tReq.ReqId, tReq.ReqType);
    return true;
	end
	...
	elseif tReq.ReqType == "REQUIREMENT_CITY_HAS_BUILDING" then -- 35, Wonders too!
		...
		if not( tSubject.SubjectType == SubjectTypes.City or tSubject.SubjectType == SubjectTypes.District ) then
			-- print("ERROR: CheckOneRequirement mismatch for subject", tSubject.SubjectType);
      dshowtable(tReq); return nil;
		end
	...
	elseif tReq.ReqType == "REQUIREMENT_PLOT_IMPROVEMENT_TYPE_MATCHES" then
		...
		if info == nil then
      -- print("ERROR: CheckOneRequirement unknown improvement", tReq.Arguments.ImprovementType);
      return false;
    end -- error
	...
	elseif tReq.ReqType == "REQUIREMENT_PLOT_ADJACENT_DISTRICT_TYPE_MATCHES" then
		...
		if info == nil then
      -- print("ERROR: CheckOneRequirement unknown district", tReq.Arguments.DistrictType);
      return false;
    end -- error
	...
	elseif tReq.ReqType == "REQUIREMENT_PLOT_IS_COASTAL_LAND" then -- new Requirement type from XP2 that can be used for Citied and Plots
        if tSubject.SubjectType == SubjectTypes.Plot or tSubject.SubjectType == SubjectTypes.City then
            ...
            -- print("ERROR: CheckOneRequirement mismatch for exp subject", "[City or Plot]", "got", tSubject.SubjectType, "req is", tReq.ReqId, tReq.ReqType);
            return true;
        end
...
function BuildCollectionOfSubjects(tMod:table, tOwner:table, sOwnerType:string)
	...
	elseif tMod.CollectionType == "COLLECTION_CITY_DISTRICTS" then
		local function AddDistrictsFromCity(tSubject:table)
			if tSubject.SubjectType ~= SubjectTypes.City then
				-- print("ERROR: BuildCollectionOfSubjects: Mismatch type for COLLECTION_CITY_DISTRICTS", sOwnerType);
        dshowtable(tMod); dshowrectable(tSubject); return
			end
			for _,district in ipairs(tSubject.Districts) do
				if district.isBuilt then
...
function ApplyEffectAndCalculateImpact(tMod:table, tSubject:table, sSubjectType:string)
	...
	local function CheckForMismatchError(sExpectedType:string)
		...
		-- print("ERROR: ApplyEffectAndCalculateImpact mismatch for subject", sSubjectType);
    dshowtable(tMod); return true;
	end
	...
	elseif tMod.EffectType == "EFFECT_ADJUST_DISTRICT_TOURISM_CHANGE" then
		if not( tSubject.SubjectType == SubjectTypes.City or tSubject.SubjectType == SubjectTypes.District ) then
			-- print("ERROR: ApplyEffectAndCalculateImpact mismatch for subject", tSubject.SubjectType);
      dshowtable(tMod); return nil;
		end
