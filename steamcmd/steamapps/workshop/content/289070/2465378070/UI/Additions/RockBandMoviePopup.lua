...
--[[ 	if (localPlayer >= 0 and ownerID == localPlayer) then
		local kResultData:table = GameInfo.Unit_RockbandResults_XP2[result];
		if (kResultData.PerformanceStars == 7) then
			UI.PlaySound("Result_Stars7_HD");
		elseif (kResultData.PerformanceStars == 6) then
			UI.PlaySound("Result_Stars6_HD");
		elseif (kResultData.PerformanceStars == 5) then
			UI.PlaySound("Result_Stars5_HD");
		elseif (kResultData.PerformanceStars == 4) then
			UI.PlaySound("Result_Stars4_HD");
		elseif (kResultData.PerformanceStars == 3) then
			UI.PlaySound("Result_Stars3_HD");
		else
			UI.PlaySound("Result_Die_HD");
		end
	end ]]
	...
	if pPlayer:IsHuman() and not GameConfiguration.IsHotseat() then
        ...
        if (GameInfo.Units[pUnit:GetType()].UnitType ~= "UNIT_ROCK_BAND") then
            return;
        end
		if pUnit ~= nil then
