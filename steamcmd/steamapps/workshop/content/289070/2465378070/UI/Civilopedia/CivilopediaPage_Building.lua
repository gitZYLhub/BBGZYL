...
PageLayouts["Building" ] = function(page)
  ...
  if #slotText > 0 then
    table.insert(greatWorkText, Locale.Lookup('LOC_TOOLTIP_GREATWORK_SOLT_TEXT'));
    for _, text in ipairs(slotText) do
      table.insert(greatWorkText, text)
    end
  end
  ...
  if #slotText == 1 and firstRowInfo and (firstRowInfo.ThemingYieldMultiplier > 0 or firstRowInfo.ThemingTourismMultiplier > 0) then
    ...
    if #themingReqText > 0 then
      table.insert(greatWorkText, Locale.Lookup('LOC_TOOLTIP_THEMING_REQ_TITLE'));
      for _, reqText in ipairs(themingReqText) do
        table.insert(greatWorkText, reqText)
      end
    end

    -- 主题化收益
    table.insert(greatWorkText, Locale.Lookup('LOC_TOOLTIP_THEMING_BONUS_TITLE'));
    if firstRowInfo.ThemingYieldMultiplier > 0 then
      table.insert(greatWorkText, '[ICON_BULLET]' .. Locale.Lookup('LOC_TOOLTIP_THEMING_YIELD_BONUS_TEXT', firstRowInfo.ThemingYieldMultiplier));
    end
    if firstRowInfo.ThemingTourismMultiplier > 0 then
      table.insert(greatWorkText, '[ICON_BULLET]' .. Locale.Lookup('LOC_TOOLTIP_THEMING_TOURISM_BONUS_TEXT', firstRowInfo.ThemingTourismMultiplier));
    end
  end
	-- ================================================================================
	...
	if #greatWorkText > 0 then
		AddRightColumnStatBox("LOC_PEDIA_HD_GREATWORKS_TEXT", function(s)
			s:AddSeparator();
			for _, text in ipairs(greatWorkText) do
				s:AddLabel(text);
			end
			s:AddSeparator();
		end)
